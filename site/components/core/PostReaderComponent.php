<?php 
    ...
    if ($action == "delete") {
        ...
        if ($userController->isUserLogged()) {
            ...
            if (($userController->getUserRoleByName($userController->getUserName()) == "Owner" || $userController->getUserRoleByName($userController->getUserName()) == "Admin") || ($userController->getUserName() == $postData["author"])) {

                // delete post
                $postsController->deletePostByID($postID);

                // redirect to forum page
                header("location: ?forum=".$forumName);
    ...
    // print post content
        ...
        // post content reader
            ...
            // add path line
            echo '<div class="color-white"><a class="basic-link" href="/">board</a> / <a class="basic-link" href="?forum='.$forumName.'">'.$forumName.'</a> / <a class="basic-link" href="#">'.$postData["name"].'</a>';
            ...
            if ($userController->isUserLogged()) {
                ...
                if (($userController->getUserRoleByName($userController->getUserName()) == "Owner" || $userController->getUserRoleByName($userController->getUserName()) == "Admin") || ($userController->getUserName() == $postData["author"])) {
                    ...
                    echo '<a href="?post='.$postID.'&forum='.$forumName.'&action=delete" class="right post-action-button">X</a>';
                }
            }
